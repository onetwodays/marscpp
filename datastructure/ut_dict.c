...
dict_t *dict_create(dict_types *type, uint32_t init_size)
{
    ...
    dt->size = dict_next_power(init_size);//capacity
    dt->mask = dt->size - 1;//capacity-1
    dt->used = 0;
...
int dict_expand(dict_t *dt, uint32_t size)
{
    ...
    for (uint32_t i = 0; i < dt->size && dt->used > 0; ++i) {
        ...
        while (entry) {
            next_entry = entry->next; //先把下一个元素保存下来，不然就找不到了
            uint32_t index = DICT_HASH_KEY(dt, entry->key) & new.mask; //重新算一下hash值
            entry->next = new.table[index]; //头插法
...
{
    if (dt->used >= dt->size * 4) //字典里元素的个数>=数组长度的4倍时,扩充
    ...
    while (entry) {
        ...
        if (entry->id < dt->id_clear) {
            if (prev) {
                ...
                dt->table[index] = next;//指向2号元素
            }
            DICT_FREE_HASH_KEY(dt, entry); //释放key
            DICT_FREE_HASH_VAL(dt, entry); //释放value
            free(entry);
            entry = NULL;
            dt->used--;
        }
...
dict_entry *dict_add(dict_t *dt, void *key, void *val)
{
    ...
    entry->next = dt->table[index];//原来的第一号变成第二号，头插法
    dt->table[index] = entry;//自己变成第一号
...
int dict_delete(dict_t *dt, const void *key)
{
    uint32_t index = DICT_HASH_KEY(dt, key) & dt->mask;
    dict_entry *entry = dt->table[index];
    dict_entry *prev = NULL; //这个有什么作用呢？

    //为什么是while循环呢？
    while (entry) {
        if (DICT_COMPARE_KEY(dt, entry->key, key) == 0) { //万一这个key不存在呢？或者这个key不是链表的第一号元素
            if (prev) {
...
dict_entry *dict_next(dict_iterator *iter)
{
    while (1) {
        if (iter->entry == NULL) {
            iter->index++;//数组下标
            if (iter->index >= iter->dt->size) //指针数组的长度
                break;
